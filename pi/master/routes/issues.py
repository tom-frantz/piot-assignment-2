...
class AllIssues(Resource):
    ...
    def get(self):
        ...
        **Admin and Engineer access.**
        ...
        try:
            all_issues_by_car = []
            car_list = issues.IssueModel.query.\
                with_entities(issues.IssueModel.car_number).\
                group_by(issues.IssueModel.car_number).all()
            for i in car_list:
                car_item = {}
                car_result = cars.CarModel.query.filter_by(car_number=i.car_number).first()
                if car_result:
                    ...
                    car_item['cost_per_hour'] = json.dumps(car_result.cost_per_hour, use_decimal=True)
                    car_item['latitude'] = json.dumps(car_result.latitude, use_decimal=True)
                    car_item['longitude'] = json.dumps(car_result.longitude, use_decimal=True)
                    car_item['lock_status'] = car_result.lock_status

                    
                    issue_result = issues.IssueModel.query.filter_by(car_number=i.car_number).all()
                    issue_list = list(
                        map(
                            lambda item: {
